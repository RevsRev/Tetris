...
class tetromino:
    ...
    types = {
        "Oshape": np.array([[1,1],[1,1]]),
        "Tshape": np.array([[0,0,0],[2,2,2],[0,2,0]]),
        "Ishape": np.array([[0,0,0,0],[3,3,3,3],[0,0,0,0],[0,0,0,0]]),
        "Lshape": np.array([[0,0,0],[4,4,4],[0,0,4]]),
        "Jshape": np.array([[0,0,0], [5,5,5], [5,0,0]]),
        "Sshape": np.array([[0,0,0],[6,6,0],[0,6,6]]),
        "Zshape": np.array([[0,0,0],[0,7,7],[7,7,0]]),
    }

    def __init__(self, type, board_location):
        ...
        self.type = type
        self.grid = tetromino.types[type]
    ...
    def rotate(self):
        ...
        self.grid = _
        self.rotaions = (self.rotations+1)%4
    ...
    def move_tetro_on_board(self, tetro, tet_dir):
        ...
        if tet_dir == "left":
            ...
            try:
                ...
                if _ == 1:
                    if tetro.type == "Ishape" and (tetro.rotations == 0 or tetro.rotations == 2):
                        self.move_tetro_on_board(tetro, "right")
                        self.move_tetro_on_board(tetro, "right")
                        try:
                            tetro2 = copy.deepcopy(tetro)
                            tetro2.rotate()
...
def play_tetris():
    ...
    tetromino_array = ["Oshape", "Tshape", "Ishape", "Lshape",
                   "Jshape", "Zshape", "Sshape"]
    while game_state ==0:
